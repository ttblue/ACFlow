...
ref_path = f'./datasets/{params.dataset}_ref.pkl'
ref_image = []
ref_mask = []
...
for i in range(num_batches):
    sam, gt, mask = sess.run(
        [model.test_sam, testset.x, testset.m])
    ref_image.append(gt)
...
ref_image = np.concatenate(ref_image, axis=0)
ref_mask = np.concatenate(ref_mask, axis=0)
# print(ref_image.shape)
# print(ref_mask.shape)
with open(ref_path, 'wb') as f:
    pickle.dump((ref_image, ref_mask), f)
