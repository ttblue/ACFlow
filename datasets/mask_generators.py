...
class CheckerboardGenerator:
    def __init__(self, block_size=16, groups=4):
        self.b = block_size
        self.g = groups
        self.gg = int(np.sqrt(self.g))

    def __call__(self, image, ind=None):
        b, gg = self.b, self.gg
        s = b // gg
        H, W, C = image.shape
        if ind is None:
            ij = np.random.randint(gg, size=2, dtype=np.int32)
        else:
            ij = list(itertools.product(range(gg), repeat=2))[ind]
        i, j = ij[0], ij[1]
        block = np.ones([b, b])
        block[i * s:(i + 1) * s, j * s:(j + 1) * s] = 0
        mask = np.tile(block, [H // b, W // b])
...
if __name__ == '__main__':
    ...
    gen = CheckerboardGenerator(16, 4)
    image = np.zeros([64, 64, 3])
    mask = gen(image)
    plt.imsave('mask.png', mask * 255)
