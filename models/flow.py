...
class Model(object):
    ...
    def inverse(self, x, m, train, z=None):
        reverse = True
        m = tf.cast(m, tf.float32)
    ...
    def build(self, trainset, validset, testset):
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
            ...
            # valid_in = rearrange(valid_x * valid_m + gray_img * (1 - valid_m))
            # tf.summary.image('valid/in', valid_in)
            # valid_out = rearrange(self.valid_sam)
            # tf.summary.image('valid/out', valid_out)
            # test_in = rearrange(test_x * test_m + gray_img * (1 - test_m))
            # tf.summary.image('test/in', test_in)
            # test_out = rearrange(self.test_sam)
            # tf.summary.image('test/out', test_out)
