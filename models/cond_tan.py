...
class Model(object):
    ...
    def embed(self, x, m, train):
        ...
        f = tf.layers.dense(res, self.hps.embed_dim)

        return f

    def forward(self, x, m, h, train, init=False):
        reverse = False
        m = tf.cast(m, tf.float32)
    ...
    def build(self, trainset, validset, testset):
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
            ...
            init_h = self.embed(self.init_x, self.init_m, True)
            self.init = self.forward(
                self.init_x, self.init_m, init_h, True, True)

            train_h = self.embed(train_x, train_m, True)
            self.train_ll = self.forward(train_x, train_m, train_h, True)
            valid_h = self.embed(valid_x, valid_m, False)
            self.valid_ll = self.forward(valid_x, valid_m, valid_h, False)
            test_h = self.embed(test_x, test_m, False)
            self.test_ll = self.forward(test_x, test_m, test_h, False)
            ...
            h_ph = self.embed(self.x_ph, self.m_ph, False)
            self.z_sam = self.sample_z(self.x_ph, self.z_ph, self.m_ph, h_ph)
